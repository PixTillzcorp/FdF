...
static t_points	*ft_newpoint(char *data, int y, int x, t_fdf *win)
{
	...
	if (!(point = (t_points *)ft_memalloc(sizeof(t_points))))
		return (NULL);
	point->x = x;
	point->y = y;
...
static t_points	**ft_new_line(char *line, int y, t_fdf *win)
{
	...
	x = 0;
	if (!(bigline = ft_memalloc(sizeof(t_points *) * (ft_strlen(line) + 1))))
		return (NULL);
...
static t_points	***ft_create_map(t_list *map, t_fdf *win)
{
	...
	ft_free_tab(tmp);
	if (!(tab = ft_memalloc(sizeof(t_points **) * ft_lstlen(map) + 1)))
		return (NULL);
	while (map)
	{
